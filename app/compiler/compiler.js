...
const { OPEN_SSL_URL, RUNTIME_URL, SETUP_DIST, PIP_MIRROR, WINDOW_RUNTIME_URL } = require('./constants');
...
function initCompile(vm) {
	...
	const {
		$compileDir,
		$compileData: { opensslName, runtimeName, opensslNameOrigin, runtimeNameOrigin }
	} = vm;
	...
	if (vm.$system.type.includes('Windows')) {
		vm.$runtimeBin = vm.$home + '\\.xigua\\python-3.7.8-embed-amd64\\python.exe';
		vm.$runtimePip = vm.$home + '\\.xigua\\python-3.7.8-embed-amd64\\Scripts\\pip.exe';
	} else {
	...
	function subUrlAsCompilePathName(vm) {
		...
		[
			...
			acc[pre.name] = fp.flowRight(fp.replace(/\.(tar|gz|tgz)/g, ''), compose)(acc[pre.url]);
...
function mixinCompile(target) {
	...
	async function compileSSl(vm) {
		...
		fp.flowRight(
			writeFile,
			injectWholeSSLScript(/\#\s+start\s+placeHolder([\s\S]*)\#\s+end\s+placeHolder/g),
			injectSSLPath(/<\%(.*)\%>/g, SETUP_DIST),
			readFileAndInitOptions
		)(target, $runtimeCompileDir + '/Modules/Setup.dist', $compiledDir);
		await execFile($runtimeShellPath, [$runtimeCompileDir, $compiledRuntimeDir]);
	}
	function writeFile(options) {
		if (options.finalContent) {
			options.target.fs.writeFileSync(options.filename, options.finalContent, 'utf-8');
		}
	}
	function injectWholeSSLScript(reg) {
		return options => {
			if (reg.test(options.content)) {
				const text = RegExp.$1;
				options.finalContent = fp.replace(text, options.replacedContent, options.content);
			}
			return options;
		}
	}
